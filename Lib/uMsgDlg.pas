...
function MessageD(const Text, Param: string; const MsgType: TMsgType;
	const Buttons: TDlgButtons; const TimeLeft: UG = DlgWait): TDlgBtn; overload;
...
const
	Captons: array[TMsgType] of string = (
		SMsgDlgConfirm,
		'Debug',
		SMsgDlgInformation,
		SMsgDlgWarning,
...
{
procedure TfMsgDlg.ButtonOpenClick(Sender: TObject);
var
	FExt: string;
	Ignore: PIgnore;
begin
	...
	if Length(FExt) > 0 then
		OpenDialogFile.Filter := '(*' + FExt + ')|*' + FExt + '|' + AllFiles
	else
		OpenDialogFile.Filter := AllFiles;
