...
begin
  FHint := Copy(Value, 1, SizeOf(IconData.szTip));
  ModifyIcon;
end;
...
//        Exit;
    ...
    begin
//      StrLCopy(PChar(IconData.szTip[0]), PChar(FHint), SizeOf(IconData.szTip)-1);
...
//      IconData.szTip[SizeOf(IconData.szTip)-1] := #0;
      ...
      Move(FHint[1], IconData.szTip, Length(FHint) * SizeOf(Char));
    end
    else
      IconData.szTip := '';
    Result := True;
...
//    StrLCopy(szInfo, PAnsiChar(aText), SizeOf(szInfo)-1);
//    StrLCopy(szInfoTitle, PAnsiChar(aTitle), SizeOf(szInfoTitle)-1);
    Move(Text[1], szInfo, Length(Text) * SizeOf(Char));
    Move(Title[1], szInfoTitle, Length(Title) * SizeOf(Char));
